...
_q_num_config(int unit)
{
    ...
    for (mport_offset = 0; mport_offset < PORTS_PER_PIPE; mport_offset++) {
        ...
        _uc_q_base[unit][lport] = uc_base;
        uc_base += bcm56560_a0_mmu_port_uc_queues(unit, lport);
    }
 ...
 */
int
bcm56560_a0_is_cblk_disabled(int unit, int clp_blk)
{
    cdk_pbmp_t clpbmp;
    int phy_port;
    int idx;
    ...
    for (idx = 0; idx < 4; idx++) {
        if (CDK_PBMP_MEMBER(clpbmp, phy_port + idx)) {
            return 1;
        }
    }
...
int
bcm56560_a0_uc_queue_num(int unit, int port, int cosq)
{
    if (port >= 0 && port < BMD_CONFIG_MAX_PORTS) {
