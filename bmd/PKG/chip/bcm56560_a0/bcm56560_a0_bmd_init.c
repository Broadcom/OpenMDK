    ...
    for (level = NODE_LVL_S1; level <= NODE_LVL_L2; level++) {
        if (level == NODE_LVL_S1) {
            ...
            for (idx_0 = 0; idx_0 <= index_max; idx_0++) {
                ...
                /* Map S1's parent node to MMU port */
                /* MMU: [CMIC],[0], [1], ..., 
                   S1 : [0],   [1], [2], ...,
                 */
            ...
            */
            ...
            for (idx_0 = 0; idx_0 <= index_max; idx_0++) { 
                ...
                if (idx_0 == CMIC_L0_IDX) {
                    ...
                    LLS_L0_CONFIGm_P_START_SPRIf_SET(l0_config, start_l1_idx + 
                        (idx_0 / 4) * LLS_NUM_L2UC);
                } else {
                    LLS_L0_CONFIGm_P_START_SPRIf_SET(l0_config, 0);
                }
            ...
            */
            ...
            for (idx_0 = 0; idx_0 <= index_max; idx_0++) { 
                ...
                if (idx_0 < start_l1_idx) {
                    ...
                    if ((sub < port_numq) && (idx < LB_MPORT) && 
                        (idx_0 >= start_l1_idx)) {
                        LLS_L1_PARENTm_C_PARENTf_SET(l1_parent, (idx * 4));
                    } else {
                ...
                /* Map L1 node to unicast/multicast entries */
                /* L1 : [8],         [20], ...,
                   L2 : UC[0..],     [12], ...,
                        MC[0x4000..],[0x400a..]
                 */             
                ...
                if (idx_0 <= 5) {
                    ...
                    if ((sub < port_numq) && (idx < LB_MPORT) && (idx_0 >= 8)) {
                        LLS_L1_CONFIGm_P_START_UC_SPRIf_SET(l1_config, 
                            (idx * LLS_NUM_L2UC) + sub);
                        LLS_L1_CONFIGm_P_START_MC_SPRIf_SET(l1_config, 
                            MC_Q_BASE + (idx * port_numq) + sub);
                    }
                }
            ...
            for (idx_0 = 0; idx_0 <= index_max; idx_0++) { 
                ...
                if ((idx_0 >= MC_Q_BASE_CMIC_PORT) && 
                    ...
                    if (idx_0 < MC_Q_BASE) {
                        ...
                        if ((sub < port_numq) && 
                            (idx_0 < (LB_MPORT * LLS_NUM_L2UC + 1))) {
                            LLS_L2_PARENTm_C_PARENTf_SET(l2_parent, (8 + idx_0));
                            ioerr += WRITE_LLS_L2_PARENTm(unit, idx_0, l2_parent);
                        }
                    } else {
                        /* multicast entries */
                        idx = (idx_0 - MC_Q_BASE) / port_numq;
                        sub = (idx_0 - MC_Q_BASE) % port_numq;
                        if ((sub < port_numq) && ((idx / port_numq) <= CMIC_MPORT)) {
                            LLS_L2_PARENTm_C_PARENTf_SET(l2_parent, (8 + (idx * LLS_NUM_L2UC) + sub));
                            ioerr += WRITE_LLS_L2_PARENTm(unit, idx_0, l2_parent);
                        }
                    }
                }
...
_cl_firmware_helper(void *ctx, uint32_t offset, uint32_t size, void *data)
{
    ...
    for (idx = 0; idx < fw_size; idx += 16) {
        ...
        for (wdx = 0; wdx <= 3; wdx++) {
            wdata = fw_entry[3 - wdx];
            CLPORT_WC_UCMEM_DATAm_SET(cl_ucmem_data, wdx^3, wdata);
        }
...
bcm56560_a0_bmd_init(int unit)
{
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ...
        qnum = bcm56560_a0_uc_queue_num(unit, port, 0);
