 ...
 */
static int
_bcm84868_ability_local_get(phy_ctrl_t *pc, uint32_t *ability, int *idx)
{
    *idx = 0;
    ...
    *(ability + 1) = 0;
    if (is_phy_repeater) {  /* check MAC interface mode */
        ...
        *(ability + 1) |= (SOC_PA_SPEED_5000MB | SOC_PA_SPEED_2500MB);
        *(ability + 1) |= PHY_ABIL_XGMII;
    } else {
        /* 2500BASE-X mode (SGMII) supports 2.5G only */
        *(ability + 1) |= SOC_PA_SPEED_2500MB;
        *(ability + 1) &= ~PHY_ABIL_XGMII;
        *(ability + 1) |= PHY_ABIL_SGMII;
    }
    return CDK_E_NONE;
}
...
_bcm84868_copper_ability_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    ...
    *ability = 0;
    ...
    if (an && an_done && link) {
        ...
        rv = _bcm84868_ability_local_get(pc, ability, arr_idx);
    }
 ...
 */
static int
_bcm84868_copper_ability_advert_set(phy_ctrl_t *pc, uint32_t ability[], int arr_idx)
{
    int ioerr = 0;
    uint32_t data;
    ...
    data &= ~MGBASE_AN_CTRL_ADV_MASK;
    data = (ability[1] & SOC_PA_SPEED_5000MB)
            ? (MGBASE_AN_CTRL_ADV_5G   | MGBASE_AN_CTRL_MG_ENABLE) : 0;
    data |= (ability[1] & SOC_PA_SPEED_2500MB)
...
static int
_bcm84868_ability_advert_set(phy_ctrl_t *pc, uint32_t ability[], int arr_idx)
{
    int rv = CDK_E_NONE;
    
    if (IS_COPPER_MODE(pc)) {
        rv = _bcm84868_copper_ability_advert_set(pc, ability, arr_idx);
    } else {
        rv = _bcm84868_fiber_ability_advert_set(pc, ability[0]);
    }
...
_bcm84868_copper_ability_advert_get(phy_ctrl_t *pc, uint32_t *ability)
{
    ...
    *ability = 0;
    ...
    if ( data & MGBASE_AN_CTRL_ADV_2P5G ) {
        *ability |= SOC_PA_SPEED_2500MB;
    }
    if ( data & MGBASE_AN_CTRL_ADV_5G ) {
        *ability |= SOC_PA_SPEED_5000MB;
    }
...
_bcm84868_medium_change(phy_ctrl_t *pc)
{
    ...
    if (changed) {
        ...
        rv = _bcm84868_ability_local_get(pc, &ability, &arr_idx);
        if (CDK_SUCCESS(rv)) {
            rv = _bcm84868_ability_advert_set(pc, &ability, arr_idx);
        }
    }
...
bcm84868_phy_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    ...
    case PhyConfig_AdvLocal:
        rv = _bcm84868_ability_local_get(pc, val, (void *)cd);
        return rv;
